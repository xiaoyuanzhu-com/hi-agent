...
It is **triggered on its own periodic clock** — every `HI_AGENT_REFLECT_EVERY` (default hourly), decoupled from the compact hot-swap. Consolidation no longer waits on the reactor session crossing its context-pressure ceiling, so a quiet or low-volume scene still consolidates on schedule instead of never (the old coupling meant no compact → no reflection). The per-scene loop carries the timer next to its pulse and alarm deadlines. It runs **detached**: nothing is held on it, so a slow consolidation never stalls the floor; the cursor makes it idempotent, and a tick that fires while a prior round is still running simply skips (the next one's frontier catches up). One consequence is accepted: a hot-swap firing between reflections may seed its replacement session from a `hot.md`/facets one cycle behind — fine, since those are projections. *Cadence* is the only knob, and it is a cost choice (every round is a paid cognition turn, plus a subprocess spawn) — not a judgment problem. A tiny frontier (< `MIN_REFLECT_SIGNALS`) is skipped rather than spending a session to file a handful of lines; `HI_AGENT_REFLECT=off` disables it entirely.

*(A scene-idle trigger — consolidating on a silence gap, the natural "the event ended, file it" moment — remains a sensible refinement on top of the periodic clock: firing on a semantic boundary rather than a fixed interval. It would live in the same per-scene loop, so the reflection guard stays a loop-local handle.)*
...
- **Semantic / idle trigger** — episodes now form on a fixed periodic clock; a scene-idle trigger would consolidate on semantic boundaries (a silence gap = "the event ended") rather than a fixed interval. It would live in the same per-scene loop, so the reflection guard stays a loop-local handle.
