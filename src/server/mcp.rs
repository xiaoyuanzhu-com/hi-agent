...
//! ([`crate::mcp`]). A POST carries one JSON-RPC message; we route by the
...
pub async fn post_mcp(
    ...
    match mcp::handle(&state.tool_registry, scene, role.as_deref(), worker_id, &msg).await {
